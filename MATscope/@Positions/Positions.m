...
%             
        ...
        function addBatch(Pos,Labels,BatchInd,adjOrder)
...
%             ordr = 1:size(Pos.List,1);
            [~,ordrInvt] = sort(interSortOrig);
            BatchInd = interBatchInd(ordrInvt); %Now sorted based on the current order of the Labels
            
            if ~isempty(adjOrder)
                interAdjInd = adjOrder(interSortIn); 
%               ordr = 1:size(Pos.List,1);
                adjOrder = interAdjInd(ordrInvt); %Now sorted based on the current order of the Labels            
            end
            ...
            for k =1:Pos.Nbatch
                ...
                if ~isempty(adjOrder)
                    adjIndx = adjOrder(indx);
                    [~,resortIndx] = sort(adjIndx);
                    indx = indx(resortIndx);
                end
                reordr = [reordr;indx];
                if k~=Pos.Nbatch
