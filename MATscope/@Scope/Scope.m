        ...
        function acqFrame(Scp,AcqData,acqname,varargin)
            ...
            if arg.usemda == 0
                for i=1:n
                    ...
                    if  Scp.FrameCount(p) >1 && mod(Scp.FrameCount(p)-1,AcqData(i).Skip)
                        skipped(i)=true;
                        continue
                    end
...
%                 memory
                ...
                builderCounter = 0; %Counter for builder coordinates (in case of skipped frames)
                % Save images
                for i = 1:n
                     % Never skip on the first frame...
                    if  Scp.FrameCount(p) >1 && mod(Scp.FrameCount(p)-1,AcqData(i).Skip)
                    ...
                    builderCounter = builderCounter +1; % increment the builder coordinates counter
                    %%% figure out image filename to be used by MM
                    filename = sprintf('img_%09g_%s_000.tif',t-1,AcqData(i).Channel);
                    if Scp.Pos.N>1 % add position folder
                        filename =  fullfile(sprintf('Pos%g',p-1),filename);
                    ...
                    builder = builder.z(0).time(0).channel(builderCounter-1).stagePosition(0);
                    ...
                    Scp.TimeStamp='ready to save image';
                    try
                        imwrite(uint16(img*2^16),fullfile(Scp.pth,acqname,filename));
                    catch  %#ok<CTCH>
                        errordlg('Cannot save image to harddeive. Check out space on drive E:');
...
%             end
            ...
            for k=1:length(AcqData)
                ...
                % Never skip on the first frame...
                if  Scp.FrameCount(p) >1 && mod(Scp.FrameCount(p)-1,AcqData(k).Skip)
                    skipped(k)=true;
                    continue
                end
        ...
        function acqname = initQueue(Scp,AcqData,varargin)
            ...
            if ~isempty(Scp.Sched)
                ...
                timeInfo = {0,arg.timespacing,0,0};
                % Choose if single Z or multi-Z
                if isempty(arg.func)
                    if isempty(arg.dz)
                        arg.func = @(tInfo) acqFramePrep(Scp,AcqData,acqname,tInfo,arg.type,'usemda',arg.usemda);
                    else
                        arg.func = @(tInfo) acqZstackPrep(Scp,AcqData,acqname,arg.dz,tInfo,arg.type,'channelfirst',arg.channelfirst,'usemda',arg.usemda);
                    end
                end
                ...
                switch arg.type
                    case 'time(site)'
                        Scp.Sched.queueReferece = {'batch','time','site'};
                    ...
                    case 'site(time)'
                        Scp.Sched.queueReferece = {'batch','site','time'};
        ...
        function returnTimeInfo = multiSiteActionPrep(Scp,func,timeInfo,liveUpdates,iterType)
            ...
            for j=1:Scp.Pos.NinBatch
                if strcmp(iterType,'site(time)')==1
                    timeInfo{4} = 0;
                end
                %%% get Skip value
                skp = Scp.Pos.getSkipForNextPosition;
                ...
                switch iterType
                    case 'time(site)'
                        Scp.Sched.addToQueue(['Move To Position i=',num2str(j)],timeInfo,-1,'MovePos',{posName},'POScallback',Scp,'xyEvent','',1,[Scp.Pos.currentBatch timeInfo{4} j  0]);
                    case 'site(time)'
                        ...
                        Scp.Sched.addToQueue(['Move To Position i=',num2str(j)],timeInfo,-1,'PrimeIter',{posName},'POScallback',Scp,'xyEvent','',1,[Scp.Pos.currentBatch j timeInfo{4} 0]);
                end
                timeInfo{3} = j;
                %%% perfrom action
                ...
                if strcmp(iterType,'site(time)')
                    if liveUpdates == 1
                        Scp.Sched.addToQueue('Update Live ROI Data',timeInfo,-1,'LiveUpdate',{},'LUcallback',Scp,'liveUpdateEvent','',1,[Scp.Pos.currentBatch j timeInfo{4} -1]);
                    end
                end
        ...
        function returnTimeInfo = multiTimepointActionPrep(Scp,func,timeInfo,liveUpdates,iterType)
            ...
            for j =1:Scp.Pos.Nbatch
                ...
                Scp.Pos.nextBatch;
                if j>1
                    Tspacing = diff(Scp.Tpnts.Tsec(1:2));
                    timeInfo{1} = Tspacing + timeInfo{1};
                end
                Scp.Sched.addToQueue(['Start batch k=',num2str(j)],timeInfo,-1,'PrimeIter',{},'POScallback',Scp,'batchEvent','',1,[Scp.Pos.currentBatch 0 0  0]);
                
                % Go through each of the timepoints and add thing to queue
            for k = 1:Scp.Tpnts.N
                if strcmp(iterType,'time(site)')==1
                    timeInfo{3} = 0;
                end
                ...
                switch iterType
                    case 'time(site)'
                        Scp.Sched.addToQueue(['Time point ',num2str(k),', T=',num2str(Tval)],timeInfo,-1,'PrimeIter',{[k,Tval,0]},'TPcallback',Scp,'absTimeEvent','',1,[Scp.Pos.currentBatch k timeInfo{3}  0]);
                    case 'site(time)'
                        Scp.Sched.addToQueue(['Time point ',num2str(k),', T_rel=',num2str(Tval),', T_abs= ',num2str(timeInfo{1})],timeInfo,-1,'TimePoint',{[k,Tval,0]},'TPcallback',Scp,'absTimeEvent','',1,[Scp.Pos.currentBatch timeInfo{3} k  0]);
                end
                timeInfo{4} = k;
                timeInfo = func(timeInfo);
                ...
                if strcmp(iterType,'time(site)')
                    if liveUpdates == 1
                        Scp.Sched.addToQueue('Update Live ROI Data',timeInfo,-1,'LiveUpdate',{iterType},'LUcallback',Scp,'liveUpdateEvent','',1,[Scp.Pos.currentBatch k timeInfo{3}  -1]);
                    end
                end
        ...
        function returnTimeInfo = acqFramePrep(Scp,AcqData,acqname,timeInfo,iterType,varargin)
            % Populate the queue with frame acuisition methods
            
            switch iterType
                case 'time(site)'
                    queueIndex = [Scp.Pos.currentBatch timeInfo{4} timeInfo{3}  -1];
                case 'site(time)'
                    queueIndex = [Scp.Pos.currentBatch timeInfo{3} timeInfo{4}  -1];
            end
            
            if nargin == 5
                Scp.Sched.addToQueue('Acquire Frame(s)',timeInfo,-1,'acqFrame',{{AcqData,acqname}},'AcqFRcallback',Scp,'acqEvent','',1,queueIndex);
            else
                inputVars = {AcqData,acqname};
                for k = 1:length(varargin)
                    inputVars{k+2} = varargin{k};
                end
                Scp.Sched.addToQueue('Acquire Frame',timeInfo,-1,'acqFrame',{inputVars},'AcqFRcallback',Scp,'acqEvent','',1,queueIndex);
            end
        ...
        function returnTimeInfo = acqZstackPrep(Scp,AcqData,acqname,dZ,timeInfo,iterType,varargin)
            ...
            Scp.Sched.addToQueue('Acquire Z Stack',timeInfo,-1,'zStack',{{AcqData,acqname,dZ,arg.channelfirst}},'Zcallback',Scp,'zStackEvent','',1,queueIndex);
...
%             notify(Scp,'LUcallback');
        ...
        function Pos = createPositions(Scp,varargin)
            ...
            for i=1:numel(ixWellsToVisit)
                ...
                for k = 1:length(WellLabels)
                    AllLabels=[AllLabels;WellLabels{k}];
                end
                if ~isempty(arg.batchinds)
                     batchVect = [batchVect;repmat(arg.batchinds(ixWellsToVisit(i)),prod(arg.sitesperwell),1)];
                end
                if ~isempty(arg.manbatchorder)
                    manOrderVect = [manOrderVect;repmat(arg.manbatchorder(ixWellsToVisit(i)),prod(arg.sitesperwell),1)];
                end
            end
            ...
            if ~isempty(arg.batchinds)
                Pos.addBatch(AllLabels,batchVect,manOrderVect);
            end
            if arg.optimize
                Pos.optimizeOrder;
            end
...
%             evalin('base','keep Scp');
        ...
        function setChannel(Scp,chnl)
            ...
            try
                Scp.mmc.setConfig(Scp.mmc.getChannelGroup,chn1);
                Scp.mmc.waitForSystem();
